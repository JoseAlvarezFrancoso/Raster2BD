...
def restart_line():
 ...
 sys.stdout.flush()
 # Funcion principal
def importar_modelos(output,archivos):
  ...
  for id_escenario in range(2,5):
   ...
   for id_periodo in range (1,4):
    ...
    for mes in range (1,13):
     ...
     archivo = archivos + "/" + modelo_name + "/" + esc_name + "/" + per_name + "/" + mes_name + ".tif"
...
if __name__ == '__main__':
 # El usuario tiene que definir una cadena con la ruta con el nombre del archivo de salida y una ruta para los archivos de entrada TIFF
 if len(sys.argv) < 3 or len(sys.argv) > 3:
  print "uso: <cadena con ruta y nombre de archivo de salida><ruta archivos TIFF>"
  raise SystemExit
 output = sys.argv[1]
 archivos = sys.argv[2]
 importar_modelos(output,archivos)
 raise SystemExit